...
* **aux\_quantiles**: auxiliary quantiles assumed known for each study variable. This is a list of length $\texttt{ncolY}$, where the aux\_quantiles[[j]] $=[0,(\tau_{j}^{q})_{q=2}^{l_{j}-1},...,1]$, i.e. there are $\ell_{j}+2$ auxiliary quantiles assumed known for each variable. Note that $\ell_{j}$, the number of auxiliary quantiles for each variable, may be unique, while $\tau^{1}_{j} = 0$ and $\tau^{\ell_{j}} = 1$. aux\_quantiles[[j]] is null, empirical deciles will be used.
* **aux\_infos**: This is a list of length $\texttt{ncolY}$ where aux\_infos[[j]] $=  [F_{j}^{-1}(0),(F_{j}^{-1}(\tau_{j}^{q}))_{q=2}^{\ell_{j}-1},...,F_{j}^{-1}(1)]$
