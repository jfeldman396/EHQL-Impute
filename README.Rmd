...
* $\texttt{aux_quantiles}$: auxiliary quantiles assumed known for each study variable. This is a list of length $\texttt{ncolY}$, where the $\texttt{aux_quantiles[[j]]} =  (0,\{\tau_{k_{j}}\}_{k=1}^{q},\dots,1\}$, i.e. there are $q_{j}+2$ auxiliary quantiles assumed known for each variable. If $\texttt{aux_quantiles[[j]]}$ is null, empirical deciles will be used. Note the quantiles for each variable do not need to be unique.
* $\texttt{aux_infos}$: This is a list of length $\texttt{ncolY}$ where $\texttt{aux_infos[[j]]} =  (F_{j}^{-1}(0),\{F_{j}^{-1}(\tau_{k_{j}})\}_{k=1}^{q},\dots,F_{j}^{-1}(1)\}$
